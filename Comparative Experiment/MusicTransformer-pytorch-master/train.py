...
for e in range(config.epochs):
    ...
    for b in range(len(dataset.files) // config.batch_size):
        ...
        train_summary_writer.add_scalar('FAD', metrics['FAD'], global_step=idx)
        ...
        if b % 100 == 0:
            ...
            eval_summary_writer.add_scalar('FAD', eval_metrics['FAD'], global_step=idx)
            eval_summary_writer.add_histogram("logits_bucket", eval_metrics['bucket'], global_step=idx)

            print('\n====================================================')
            print('Epoch/Batch: {}/{}'.format(e, b))
            print('Train >>>> Loss: {:6.6}, Accuracy: {}, FAD:{}'.format(metrics['loss'], metrics['accuracy'], metrics['FAD']))
            print('Eval >>>> Loss: {:6.6}, Accuracy: {}, FAD:{}'.format(eval_metrics['loss'], eval_metrics['accuracy'], eval_metrics['FAD']))
