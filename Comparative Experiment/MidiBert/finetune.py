...
def get_args_finetune():
    ...
    ### dataset & data root ###
    parser.add_argument(
        '--dataset', choices=['asap', 'Pianist8', 'POP909'], required=True)
    parser.add_argument('--dataroot', type=str, default=None)
    ### path setup ###
    parser.add_argument('--dict_file', type=str,
                        default='../../Data/Octuple.pkl')
    parser.add_argument('--name', type=str, default='')
    parser.add_argument(
        '--ckpt', default='result/pretrain/midibert/model.ckpt')
        ...
        if model != None:  # load model
            ...
            if SeqClass:
                self.model = SequenceClassification(
                    self.midibert, class_num, hs).to(self.device)
            else:
                self.model = TokenClassification(
                    self.midibert,  class_num+1, hs).to(self.device)
    ...
    def iteration(self, training_data, mode, seq):
        ...
        for x, y in pbar:  # (batch, 512, 768)
            ...
            if not seq:
                ...
                attn = torch.ones((batch, 1024)).to(
                    self.device)  # attend each of them

            # seq: (batch, class_num) / token: (batch, 512, class_num)
            y_hat = self.model.forward(x, attn, self.layer)
...
def load_data_finetune(dataset, task, data_root=None):
    ...
    if task == "gen":
        ...
        if dataset == 'pop909' or dataset == 'POP909':
            ...
            y_train = np.load(os.path.join(
                data_root, f'{dataset}_train_comans.npy'), allow_pickle=True)
            y_val = np.load(os.path.join(
                data_root, f'{dataset}_valid_comans.npy'), allow_pickle=True)
            y_test = np.load(os.path.join(
                data_root, f'{dataset}_test_comans.npy'), allow_pickle=True)
